...
function startup() {
  //on startup set brushcolor to white, then wait for a change in the colorpicker
...
function changeBrushColor(event) {
  //when there is a change in the colorpicker, grab the event and set the color of the brush to the color in the colorpicker
...
function changeBrushSize(event) {
  //when there is a change in the value of brush size input, set the brush variable to the value of the input
  brush = event.target.value;
  console.log('changing brushsize to ' + brush);
}

function preload() {
  //loads before setup() makes sure i can recieve data from server socket
  socket = io.connect(); //this page comes from server source, so no arguments are needed
  socket.on('welcome', data => {
    //displays welcome message in console to test if connection is working
    console.log(data);
  });
  socket.on('drawing', data => {
    //recieves array from server with history of drawings
    console.log(data);
    for (var index = 0; index < data.length; index++) {
      //for every already drawn object, draw it in the empty canvas with the data variables
      noStroke();
...
function setup() {
  //This code runs once to configure the socket.io and create the canvas
...
function saveDrawing() {
  //saves the canvas to the users pc
  console.log('Saving canvas');
  saveCanvas('myDrawing', 'jpg');
}

function mouseDragged() {
  //when the mouse is clicked and dragged across the canvas, send the x/y/color data to the socket
  console.log(
    'Sending: ' +
      mouseX +
      ' ' +
...
function drawEllipse() {
  //Drawing a simple ellipse at mouse location
